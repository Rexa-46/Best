...
/* ---------------------------------------------------------
   ...
   شماره کارت ۱۶ رقمی، تاریخ انقضا (سمت راست) و رنگ‌بندی مطابق بانک
--------------------------------------------------------- */
const enDigits = (v) => String(v || "").replace(/[۰-۹]/g, (d) => "۰۱۲۳۴۵۶۷۸۹".indexOf(d)).replace(/[٠-٩]/g, (d) => "٠١٢٣٤٥٦٧٨٩".indexOf(d));
const cleanCardNumber = (v) => enDigits(v).replace(/\D/g, "").slice(0, 16);
const groupCardNumber = (v) => cleanCardNumber(v).replace(/(.{4})/g, "$1 ").trim();
const cleanExpiry = (v) => {
  const d = enDigits(v).replace(/\D/g, "").slice(0, 4);
  return d.length > 2 ? `${d.slice(0, 2)}/${d.slice(2)}` : d;
};
const isValidExpiry = (v) => /^\d{2}\/\d{2}$/.test(v) && Number(v.slice(3)) >= 1 && Number(v.slice(3)) <= 12; // سال/ماه مثل کارت‌های ایرانی

// رنگ‌های تقریبی هر بانک (لوگوی رسمی استفاده نشده)
const BANKS = [
  { id: "tosee-saderat", name: "توسعه صادرات", kw: ["توسعه صادرات"], bins: ["627648"], from: "#0B4F8A", to: "#062B4D", accent: "#7BC142" },
  { id: "saderat", name: "صادرات", kw: ["صادرات"], bins: ["603769"], from: "#0B5EA8", to: "#06325C", accent: "#F58220" },
  { id: "melli", name: "ملی", kw: ["ملی"], bins: ["603799"], from: "#0F6E8C", to: "#0A3D52", accent: "#F2C230" },
  { id: "mellat", name: "ملت", kw: ["ملت"], bins: ["610433", "991975"], from: "#D42132", to: "#8C0F1E", accent: "#FFFFFF" },
  { id: "tejarat", name: "تجارت", kw: ["تجارت"], bins: ["627353", "585983"], from: "#1355B3", to: "#0A2C6B", accent: "#3DB4F2" },
  { id: "sepah", name: "سپه", kw: ["سپه"], bins: ["589210"], from: "#F28C1B", to: "#B4570A", accent: "#1F4E9C" },
  { id: "refah", name: "رفاه کارگران", kw: ["رفاه"], bins: ["589463"], from: "#0DA0DB", to: "#065F87", accent: "#FFFFFF" },
  { id: "keshavarzi", name: "کشاورزی", kw: ["کشاورزی"], bins: ["603770", "639217"], from: "#2F8F3E", to: "#175A22", accent: "#F2C230" },
  { id: "maskan", name: "مسکن", kw: ["مسکن"], bins: ["628023"], from: "#E8792B", to: "#A8480C", accent: "#FFFFFF" },
  { id: "pasargad", name: "پاسارگاد", kw: ["پاسارگاد"], bins: ["502229", "639347"], from: "#B5892A", to: "#5E4310", accent: "#111111" },
  { id: "parsian", name: "پارسیان", kw: ["پارسیان"], bins: ["622106", "639194"], from: "#9C1D45", to: "#5A0E27", accent: "#F2C230" },
  { id: "saman", name: "سامان", kw: ["سامان"], bins: ["621986"], from: "#0E7AC4", to: "#074A7D", accent: "#5DD0F5" },
  { id: "eghtesad-novin", name: "اقتصاد نوین", kw: ["اقتصاد نوین", "اقتصادنوین"], bins: ["627412"], from: "#6B2A9A", to: "#3A1257", accent: "#F2C230" },
  { id: "sina", name: "سینا", kw: ["سینا"], bins: ["639346"], from: "#1A73C9", to: "#0B3F73", accent: "#FFFFFF" },
  { id: "shahr", name: "شهر", kw: ["بانک شهر"], bins: ["502806", "504706"], from: "#E0342B", to: "#8F1610", accent: "#FFFFFF" },
  { id: "ayandeh", name: "آینده", kw: ["آینده"], bins: ["636214"], from: "#D8632A", to: "#8A3410", accent: "#F2C230" },
  { id: "dey", name: "دی", kw: ["بانک دی"], bins: ["502938"], from: "#159AA6", to: "#0A5C64", accent: "#FFFFFF" },
  { id: "karafarin", name: "کارآفرین", kw: ["کارآفرین"], bins: ["627488"], from: "#0F8C63", to: "#095238", accent: "#F2C230" },
  { id: "post", name: "پست بانک", kw: ["پست بانک", "پستبانک"], bins: ["627760"], from: "#1E9A45", to: "#0F5C28", accent: "#F2C230" },
  { id: "sarmayeh", name: "سرمایه", kw: ["سرمایه"], bins: ["639607"], from: "#173A73", to: "#0A1F45", accent: "#F2C230" },
  { id: "ansar", name: "انصار", kw: ["انصار"], bins: ["627381"], from: "#2A5DA8", to: "#122E5A", accent: "#F2C230" },
  { id: "mehr-iran", name: "مهر ایران", kw: ["مهر"], bins: ["606373"], from: "#2EA043", to: "#17612A", accent: "#FFFFFF" },
  { id: "tosee-taavon", name: "توسعه تعاون", kw: ["توسعه تعاون"], bins: ["502908"], from: "#1F7A8C", to: "#0D4450", accent: "#F2C230" },
  { id: "sanat-madan", name: "صنعت و معدن", kw: ["صنعت و معدن"], bins: ["627961"], from: "#3B6E4F", to: "#1E3D2A", accent: "#F2C230" },
  { id: "gardeshgari", name: "گردشگری", kw: ["گردشگری"], bins: ["505416"], from: "#2B8CBE", to: "#145070", accent: "#F2C230" },
  { id: "markazi", name: "مرکزی", kw: ["بانک مرکزی"], bins: ["636795"], from: "#26466D", to: "#111F33", accent: "#F2C230" },
];
function detectBank(account) {
  const num = cleanCardNumber(account.cardNumber);
  if (num.length >= 6) { const b = BANKS.find((x) => x.bins.includes(num.slice(0, 6))); if (b) return b; }
  const n = String(account.name || "");
  return BANKS.find((b) => b.kw.some((k) => n.includes(k))) || null;
}

function BankCard({ account, balance, hidden, currency, usdRate }) {
  const bank = detectBank(account);
  const num = cleanCardNumber(account.cardNumber);
  const last4 = num.length === 16 ? num.slice(-4) : account.cardNumberLast4;
  const shownNumber = num.length === 16 ? groupCardNumber(num) : `•••• •••• •••• ${last4 || "••••"}`;
  const from = bank?.from || BRAND.violet, to = bank?.to || BRAND.header, accent = bank?.accent || "#ffffff";
  return (
    <div style={{
      minWidth: 300, maxWidth: 300, height: 176, borderRadius: 18, padding: "16px 20px", color: "#fff", flexShrink: 0,
      ...
      {bank && <div style={{ position: "absolute", bottom: -70, right: -50, width: 190, height: 190, borderRadius: "50%", background: accent, opacity: 0.16 }} />}
      {bank && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 4, background: accent, opacity: 0.85 }} />}
      ...
      <div style={{ zIndex: 1 }}>
        <div style={{ fontSize: 18, letterSpacing: 2, fontWeight: 700, direction: "ltr", textAlign: "left", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>
          {shownNumber}
        </div>
        {/* تاریخ انقضا — سمت راست کارت */}
        {account.cardExpiry && <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 4, direction: "ltr" }}>
          <div style={{ textAlign: "right", lineHeight: 1.2 }}>
            <div style={{ fontSize: 8.5, opacity: 0.75, fontWeight: 600 }}>تاریخ انقضا</div>
            <div style={{ fontSize: 14, fontWeight: 800, letterSpacing: 1, fontVariantNumeric: "tabular-nums" }}>{account.cardExpiry}</div>
          </div>
        </div>}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", zIndex: 1 }}>
        <div style={{ fontSize: 11, opacity: 0.85 }}>{bank ? `بانک ${bank.name}` : account.type === "card" ? "کارت" : "بانک"}</div>
...
function SmileFrownArc({ income, expense }) {
  ...
  // لبخند = U (دهانه رو به بالا) ، اخم = ∩ (دهانه رو به پایین)
  const d = net > 0
    ? `M 18 ${h * 0.30} Q ${w / 2} ${h * 0.90} ${w - 18} ${h * 0.30}`
    : net < 0
      ? `M 18 ${h * 0.70} Q ${w / 2} ${h * 0.14} ${w - 18} ${h * 0.70}`
      : `M 18 ${h / 2} L ${w - 18} ${h / 2}`;
  return (
...
// وقتی کیبورد باز می‌شود و ارتفاع صفحه کم می‌شود، مقدار کم‌شدن را برمی‌گرداند
// تا منو با همان مقدار پایین برود و بالای کیبورد نیاید (زیر کیبورد بماند).
function useKeyboardOffset() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (!editing() || h > base) base = h;          // بدون کیبورد: ارتفاع پایه را به‌روز کن
      const diff = base - h;
      setOffset(editing() && diff > 120 ? diff : 0);  // فقط وقتی کیبورد واقعاً باز است
    };
    const later = () => { update(); setTimeout(update, 120); setTimeout(update, 400); };
    const reset = () => { base = window.innerHeight; later(); };
...
function BottomNav({ active, setActive, onAdd }) {
  ...
  return <div style={{ position: "fixed", left: "50%", transform: `translate(-50%, ${kbOffset}px)`, bottom: 0, width: "min(480px, 100vw)", minHeight: 72, padding: "7px 6px calc(7px + env(safe-area-inset-bottom, 0px))", background: t.card, borderTop: `1px solid ${t.border}`, display: "grid", gridTemplateColumns: "1fr 1fr 64px 1fr 1fr", alignItems: "center", zIndex: 300, boxShadow: "0 -5px 18px rgba(0,0,0,.10)" }}>
...
function AccountsManager({ accounts, addAccount, deleteAccount, updateAccount, accountBalance, favorites, toggleFavorite }) {
  const st = useStyles();
  const [name, setName] = useState(""); const [type, setType] = useState("bank"); const [initial, setInitial] = useState(""); const [cardNo, setCardNo] = useState(""); const [expiry, setExpiry] = useState(""); const [formErr, setFormErr] = useState("");
  const [editingId, setEditingId] = useState(null); const [editBalance, setEditBalance] = useState(""); const [editCardNo, setEditCardNo] = useState(""); const [editExpiry, setEditExpiry] = useState(""); const [editErr, setEditErr] = useState("");
  return <div>
    <div style={{ ...st.card, padding: 14, marginBottom: 16 }}>
      ...
      {(type === "bank" || type === "card") && <>
        <input placeholder="شماره کارت ۱۶ رقمی" value={groupCardNumber(cardNo)} onChange={(e) => setCardNo(cleanCardNumber(e.target.value))} style={{ ...st.input, direction: "ltr", textAlign: "right", letterSpacing: 1 }} inputMode="numeric" />
        <input placeholder="تاریخ انقضا (سال/ماه، مثل 05/09)" value={expiry} onChange={(e) => setExpiry(cleanExpiry(e.target.value))} style={{ ...st.input, direction: "ltr", textAlign: "right" }} inputMode="numeric" />
      </>}
      {formErr && <div style={{ color: BRAND.crimson, fontSize: 12, fontWeight: 700, marginBottom: 8 }}>{formErr}</div>}
      <button onClick={() => {
        if (!name.trim()) return;
        if ((type === "bank" || type === "card") && cardNo && cardNo.length !== 16) { setFormErr("شماره کارت باید ۱۶ رقم باشد"); return; }
        if (expiry && !isValidExpiry(expiry)) { setFormErr("تاریخ انقضا را به شکل سال/ماه وارد کنید (مثل 05/09)"); return; }
        setFormErr("");
        addAccount({ name: name.trim(), type, initial: Number(initial || 0), cardNumber: cardNo || undefined, cardNumberLast4: cardNo ? cardNo.slice(-4) : undefined, cardExpiry: expiry || undefined });
        setName(""); setInitial(""); setCardNo(""); setExpiry("");
      }} style={st.primaryBtn}>افزودن</button>
    </div>
    <div style={{ ...st.card, padding: "4px 12px" }}>
      ...
      {accounts.map((a) => <div key={a.id}>
        <Row title={a.name} subtitle={`${a.type === "bank" ? "بانک" : a.type === "card" ? "کارت" : "صندوق"} · موجودی: ${toFaInt(accountBalance(a.id))} ریال`}
          extra={<div style={{ display: "flex", gap: 4 }}>
            <button onClick={() => { setEditingId(a.id); setEditBalance(String(accountBalance(a.id))); setEditCardNo(a.cardNumber || ""); setEditExpiry(a.cardExpiry || ""); setEditErr(""); }} style={{ ...miniBtn, color: BRAND.violet }}><Pencil size={13} /></button>
        ...
        {editingId === a.id && (a.type === "bank" || a.type === "card") && <div style={{ padding: "0 0 10px" }}>
          <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
            <input placeholder="شماره کارت ۱۶ رقمی" value={groupCardNumber(editCardNo)} onChange={(e) => setEditCardNo(cleanCardNumber(e.target.value))} style={{ ...st.input, margin: 0, flex: 2, direction: "ltr", textAlign: "right" }} inputMode="numeric" />
            <input placeholder="سال/ماه" value={editExpiry} onChange={(e) => setEditExpiry(cleanExpiry(e.target.value))} style={{ ...st.input, margin: 0, flex: 1, direction: "ltr", textAlign: "right" }} inputMode="numeric" />
          </div>
          {editErr && <div style={{ color: BRAND.crimson, fontSize: 12, fontWeight: 700, marginBottom: 6 }}>{editErr}</div>}
          <button onClick={() => {
            if (editCardNo && editCardNo.length !== 16) { setEditErr("شماره کارت باید ۱۶ رقم باشد"); return; }
            if (editExpiry && !isValidExpiry(editExpiry)) { setEditErr("تاریخ انقضا باید به شکل سال/ماه باشد (مثل 05/09)"); return; }
            updateAccount?.(a.id, { cardNumber: editCardNo || undefined, cardNumberLast4: editCardNo ? editCardNo.slice(-4) : a.cardNumberLast4, cardExpiry: editExpiry || undefined });
            setEditingId(null);
          }} style={st.primaryBtn}>ذخیره اطلاعات کارت</button>
        </div>}
      </div>)}
    </div>
  </div>;
